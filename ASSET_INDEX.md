...
| `master-satellite-sync-protocol` | Constitution | `.LinkStrategy/04_SYNC_PROTOCOL.md` | Brain | Active | Defines technical protocols for Force Sync (Rules) and Asset Harvesting (Code). | Sync or isolation management. | Reliable synchronization and protection. | Mandatory for Master-Satellite operations. | `docs/sync-linkage.md`, `scripts/push-rules-to-satellite.ps1` |
...
| `auditor-capability-engine` | Constitution | `.LinkStrategy/05_AUDITOR_CAPABILITY_ENGINE.md` | Brain | Active | Defines the core logic of the Auditor as an organizational engine, maturity models, and output standards. | Strategic decision on scaling Layer 1. | Auditor performance standards. | Required for Layer 1 architecture. | `.LinkStrategy/00_BLUEPRINT_Link Strategy.md` |
| `auditor-recruitment-training` | Constitution | `.LinkStrategy/06_AUDITOR_RECRUITMENT_TRAINING.md` | Brain | Active | Defines the practical recruitment personas, AI-augmented training modules, and verification gates for onboarding. | Recruitment or onboarding event. | Field-ready Auditor. | Mandatory for New Auditor onboarding. | `.LinkStrategy/05_AUDITOR_CAPABILITY_ENGINE.md` |
...
| `ls-workflow-new-project` | Workflow | `.agents/workflows/ls-workflow-new-project.md` | Brain | Draft | Step-by-step for setting up a brand new project. | New project request. | Initialized project folder. | Required for project consistency. | `scripts/new-project.ps1` |
...
| `init-satellite` | Script | `scripts/init-satellite.ps1` | Brain | Active | Automates GitHub repo creation, remote setup, and initial governance push. | Project path, Repo name. | Production-ready Satellite Repository. | Required for external project onboarding. | `docs/sync-linkage.md` |
...
| `push-rules` | Script | `scripts/push-rules-to-satellite.ps1` | Brain | Active | Pushes governance assets from Master to Satellite. | Project path. | Updated rules in satellite. | Post-governance update. | `docs/sync-linkage.md` |
| `pull-code` | Script | `scripts/pull-code-from-satellite.ps1` | Brain | Active | Pulls (harvests) source code from Satellite to Master. | Project path, Remote URL. | Source code integrated in Master. | Post-milestone acceptance. | `docs/sync-linkage.md` |
