...
5. **Package Contract:** Được sửa `package.json` để thêm dependency, metadata và `test` script. Không được xóa/đổi `verify-gate`, `ls-gitpush`; không expose Brain-only scripts: `new-project`, `new-hands`, `push-rules`, `pull-code`, `init-satellite`, `self-test`, `stress-test`.
6. **Tool-Only Delivery:** Không push thủ công. Chỉ nộp bằng `npm run ls-gitpush`; tool sẽ push trực tiếp lên `origin/main`.
7. **Spec Baseline:** `01_TASK_SPEC.md` là contract gốc và task list baseline. Không dùng file này làm progress log.
