...
| `new-project` | Script | `npm run new-project` | Brain | Active | Generates a Brain Project Workspace skeleton from Master. | `--client-id`, `--project-name`, `--project-type`, `--base-path`. | `../[CLIENT_ID]-[PROJECT_NAME]/`. | Required for consistent project creation. | `ls-workflow-new-project.md` |
...
| `pull-code` | Script | `npm run pull-code` | Brain | Active | CI-gated harvest from Satellite `main` to Brain monorepo. | `--project-path`, optional `--remote-url`, `--dry-run`, `--skip-ci-check`. | Source code integrated in Master only after remote gate success. | Post-milestone Brain harvest. | `docs/sync-linkage.md` |
