...
| Name | Type | Path | Owner | Status | Purpose | Input | Output | Mandatory Usage | Related Docs |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `ls-rule-master-governance` | Rule | `.agents/rules/1.md` | Brain | Placeholder | Planned master rule for Brain sovereignty, Spec-First execution, verification gate, hardening, and audit. Current file is still placeholder and must be renamed/hardened. | Agent session or delivery task. | Operational constraints for execution. | Planned as mandatory for all work. | `GEMINI.md`, `backlog.md#p13---doi-ten-placeholder-rule` |
| `ls-rule-secret-management` | Rule | `.agents/rules/ls-rule-secret-management.md` | Brain | Planned | Secret handling policy for `.env`, credentials, production access, and key revocation. | Any project using environment variables or credentials. | Secure secret usage and revocation requirements. | Required before project/satellite handover. | `.gitignore`, `.LinkStrategy/03_HANDOVER_SPEC.md` |

## Workflow Registry

| Name | Type | Path | Owner | Status | Purpose | Input | Output | Mandatory Usage | Related Docs |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `ls-workflow-delivery-loop` | Workflow | `.agents/workflows/1.md` | Brain | Placeholder | Planned workflow for the full SDLC loop: Spec-First, Bidding & Isolation, Execution, Gate, Hardening. Current file is still placeholder and must be renamed/hardened. | Task/module delivery request. | Standard execution sequence. | Planned as mandatory for all delivery work. | `.LinkStrategy/01_SOP_LINK_STRATEGY.md`, `backlog.md#p14---doi-ten-placeholder-workflow` |
...
| Name | Type | Path | Owner | Status | Purpose | Input | Output | Mandatory Usage | Related Docs |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `task-spec-template` | Template | `.agents/templates/01_TASK_SPEC_TEMPLATE.md` | Brain | Draft | Provides the base Spec-First structure for task/module handover. | Feature/module objective and context. | `01_TASK_SPEC.md`. | Required before implementation starts. | `.LinkStrategy/03_HANDOVER_SPEC.md` |
| `qa-logs-template` | Template | `.agents/templates/02_QA_LOGS_TEMPLATE.md` | Brain | Placeholder | Planned communication-plane template for questions, decisions, owners, and status. Current file is empty. | Questions and decisions during delivery. | `02_QA_LOGS.md`. | Required for all project/module work once completed. | `.LinkStrategy/02_FULL_SYSTEM_CONFIGURATION.md` |
...
| `hardening-proposal-template` | Template | `.agents/templates/HARDENING_PROPOSAL_TEMPLATE.md` | Brain | Planned | Captures reusable logic candidates and abstraction plan after PR/milestone. | Completed implementation or module. | Hardening proposal for asset extraction. | Required before macro-hardening review. | `backlog.md`, `.LinkStrategy/01_SOP_LINK_STRATEGY.md` |
| `gate-scorecard-template` | Template | `.agents/templates/GATE_SCORECARD_TEMPLATE.md` | Brain | Planned | Standard 100-point delivery scorecard. | Test/lint/docs/security/hardening evidence. | Gate score and payment decision basis. | Required for delivery acceptance. | `.LinkStrategy/01_SOP_LINK_STRATEGY.md` |
...
| Name | Type | Path | Owner | Status | Purpose | Input | Output | Mandatory Usage | Related Docs |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `scripts-catalog` | Script | `scripts/README.md` | Brain | Placeholder | Planned catalog for platform automation scripts. Current file is empty. | Script user or maintainer. | Script usage guidance. | Required before adding automation scripts. | `README.md` |
| `new-project` | Script | `scripts/new-project.ps1` | Brain | Planned | Generates project workspace skeleton. | `client_id`, `project_name`, `project_type`. | `projects/[CLIENT_ID]-[PROJECT_NAME]/`. | Required for consistent project creation once implemented. | `README.md`, `backlog.md` |
| `new-module` | Script | `scripts/new-module.ps1` | Brain | Planned | Generates module workspace inside a project. | `project_path`, `module_name`, `module_type`. | Module folder with docs/src/tests/logs. | Required for module-based tasking once implemented. | `.LinkStrategy/03_HANDOVER_SPEC.md` |
| `verify-gate` | Script | `scripts/verify-gate.ps1` | Brain | Planned | Performs local gate checks and creates pass/fail evidence. | Project or module path. | Gate report. | Required before acceptance once implemented. | `gate-scorecard-template` |
...
| `demo-base-platform` | Project | `projects/DEMO-BASE-PLATFORM/` | Brain | Planned | Reference project proving project factory, templates, logs, and gate verification. | Project factory scripts and templates. | Demo project workspace. | Required for Base Platform V1 validation. | `backlog.md` |
